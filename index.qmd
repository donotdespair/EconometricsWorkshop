...
> Verifying Sources of Identification of Structural Vector Autoregressions Using the BETEL Framework

18:30 - 21:00 | *Dinner at*
...
> Guo Yan
>
> We study the use of the support vector machine (SVM), a widely used machine learning classification method, for estimation and prediction in the context of binary choice models (BCM). We establish the asymptotics of the linear and kernel SVM separately, corresponding to BCMs with a linear and a nonparametric systematic component of covariates X, respectively. In the context of BCM with a nonparametric function of X, the kernel SVM cannot be used for estimation, whereas it does provide an optimal classification rule for prediction asymptotically. In the context of a class of BCM with a linear systematic function of X, the linear SVM is optimal for prediction if and only if it can consistently estimate the linear coefficient. Though generally inconsistent, the linear SVM is √n consistent under certain conditions on some specific distributional aspects of covariates and the error term. Under a symmetry condition on the distribution of X for consistency, we show that linear SVM can be more efficient than the most efficient estimator without exploiting any distributional information on X. 
